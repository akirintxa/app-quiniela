...
export default async function RankingPage({
  ...
  // Traemos todos los partidos finalizados para calcular la tendencia
  ...
  const lastMatchId = allFinishedMatches?.[0]?.id;
  ...
  // 4. Calculate Trends
  // Ranking A: Todos los puntos actuales (incluyendo live)
  // Ranking B: Puntos hasta el penúltimo partido finalizado
  ...
  predictionsData?.forEach(pred => {
    if (currentScores[pred.user_id] !== undefined) {
      currentScores[pred.user_id] += pred.points_won || 0;
      
      // Para el ranking previo, excluimos el último partido finalizado
      if (pred.match_id !== lastMatchId) {
        // También nos aseguramos de que solo contamos partidos finalizados para la tendencia estable
        const matchIsFinished = allFinishedMatches?.some(m => m.id === pred.match_id);
        if (matchIsFinished && pred.match_id !== lastMatchId) {
          previousScores[pred.user_id] += pred.points_won || 0;
        }
      }
    }
  });

  // Sort Current (Para la posición en la tabla)
  const currentRanked = Object.keys(currentScores)
    .map(id => ({ id, points: currentScores[id] }))
    .sort((a, b) => b.points - a.points);

  // Sort Previous (Para calcular la tendencia)
  // IMPORTANTE: El ranking previo SOLO debe incluir partidos finalizados
  const previousRanked = Object.keys(previousScores)
    .map(id => ({ id, points: previousScores[id] }))
    .sort((a, b) => b.points - a.points);

  // Process Final User Map
  const userMap: Record<string, any> = {};
  profilesData?.forEach(p => {
    const currentPos = currentRanked.findIndex(r => r.id === p.id);
    const previousPos = previousRanked.findIndex(r => r.id === p.id);
  ...
  if (viewUserId && historyProfile) {
    ...
    userHistory = (matches || []).map(m => {
      ...
      cumulative += pts;
      return {
        match: `${(m.team_a as any).name.substring(0,3).toUpperCase()} vs ${(m.team_b as any).name.substring(0,3).toUpperCase()}`,
          ...
          <div className="bg-white dark:bg-zinc-900 w-full max-w-xl max-h-[85vh] rounded-[3rem] shadow-2xl overflow-hidden flex flex-col border border-gray-100 dark:border-zinc-800">
            <div className="p-6 sm:p-8 border-b border-gray-50 dark:border-zinc-800 flex justify-between items-center bg-gray-50/50 dark:bg-zinc-800/50">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="font-black text-xl uppercase tracking-tighter">{historyProfile.nickname}</h3>
                  <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest">Resumen de Jornadas</p>
                </div>
              </div>
                  ...
                  <div key={i} className="flex items-center justify-between bg-gray-50 dark:bg-zinc-800/40 p-4 rounded-2xl border border-gray-100 dark:border-zinc-800/50">
                    <div className="flex flex-col">
                      ...
                      <div className="flex items-center gap-3">
                        <span className="text-xs font-bold text-gray-500 italic">Pred: <span className="text-gray-900 dark:text-white not-italic">{h.pred}</span></span>
                        <span className="w-px h-3 bg-gray-200"></span>
                        <span className="text-xs font-bold text-gray-500 italic">Res: <span className="text-blue-600 not-italic">{h.res}</span></span>
                      </div>
                    </div>
                    <div className="text-right">
                      <div className="font-black text-lg tracking-tighter leading-none">{h.total} <span className="text-[10px] text-gray-400">PTS</span></div>
                      <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded-full ${h.pts > 0 ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-400'}`}>+{h.pts}</span>
                    </div>
                  </div>
                ))}
                    ...
                    <Link href={selectedPoolId ? `/ranking?pool=${selectedPoolId}&view_user=${member.id}` : `/ranking?view_user=${member.id}`} className={`flex items-center w-full px-6 py-5 transition-all group ${member.isMe ? 'bg-blue-600 text-white' : 'hover:bg-gray-50/50 dark:hover:bg-zinc-800/30'}`}>
                      <div className="w-16 text-center flex-shrink-0 flex flex-col items-center">
                        ...
                        <div className="mt-1 flex items-center justify-center h-4">
                          {member.trend === 'up' && (
                            <div className="flex flex-col items-center animate-in slide-in-from-bottom-1 duration-500">
                              <span className="text-[12px] text-green-500 leading-none">▲</span>
                            </div>
                          )}
                          {member.trend === 'down' && (
                            <div className="flex flex-col items-center animate-in slide-in-from-top-1 duration-500">
                              <span className="text-[12px] text-red-500 leading-none">▼</span>
                            </div>
                          )}
                      ...
                      <div className="flex-1 flex items-center gap-4">
                        <div className={`w-12 h-12 rounded-2xl overflow-hidden border-2 ${member.isMe ? 'border-white/30' : 'border-gray-100'}`}>
                          <img src={member.avatar} alt="av" className="w-full h-full object-cover" />
                        </div>
                        <div className="flex flex-col">
                          <div className="flex items-center gap-2">
                            <span className="font-black text-sm uppercase tracking-tighter">{member.nickname}</span>
                            {member.flag && <img src={getFlagUrl(member.flag) || ""} alt="f" className="w-4 h-3 rounded-[2px] shadow-sm" />}
                          </div>
                          <span className={`text-[8px] font-black uppercase tracking-widest ${member.isMe ? 'opacity-60' : 'text-gray-400 group-hover:text-blue-500'}`}>Ver Historial</span>
                        </div>
                      </div>
